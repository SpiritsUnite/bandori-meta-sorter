...
function band_bp(band, event) {
    ...
    for (let card of band) {
        ...
        if (!attr_v[card.attr])
            attr_v[card.attr] = 0;
        attr_v[card.attr] += bp;
        if (event_attr == card.attr)
            event_bonus += bp * EVENT_ATTR_MULT;
        if (event_mem.includes(card.characterId))
            event_bonus += bp * EVENT_MEM_MULT;
...
async function card_init() {
    ...
    let [jp_cards, en_cards] = (await Promise.all([
        responses[0].json(),
        responses[1].json()
    ])).map(x => x.data);
    ...
    for (let i = 1; i <= names.length; i++)
        card_sels[i] = document.getElementById(`card-picker-${i}`);
    ...
    let opener;
    document.querySelectorAll(".card-btn").forEach(element => element.addEventListener("click", event => {
        opener = element;
        modal.modal("show");
    }));
    document.querySelectorAll(".card-del").forEach(element => element.addEventListener("click", event => {
        let card_btn = element.previousElementSibling;
        card_btn.value = "-1";
        card_btn.textContent = "Select card...";
    }));
    for (let i = 1; i <= names.length; i++) {
        card_sels[i].addEventListener("change", event => {
            if (card_sels[i].value == "-1")
                return;
            ...
            let skill_sel = document.getElementById(`skill${opener_id}`);
    ...
    for (let i = 0; i < 5; i++) {
        let card_button = document.getElementById(`card${i}`);

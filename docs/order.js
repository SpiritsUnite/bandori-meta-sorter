...
var order;
(function (order_1) {
    const song_sel = document.getElementById("song");
    ...
    function init_song_list(display) {
        song_sel.innerHTML = "";
        let titles = song_data.map((song) => [song, locale_title(song, display)]).filter((x) => x[1] !== null);
    ...
    function add_orders(skills, options, sel) {
        ...
        if (!chart)
            return;
        let display_fn = (skill, idx) => `${skill_string(skill)} (${chart.skill[idx][skill.sl]})`;

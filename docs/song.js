...
function skill_perms(skills) {
    let ret = [];
    let cmp = (l, r) => l.mult - r.mult || l.sl - r.sl;
    skills = skills.slice();
    skills.sort(cmp);
...
function perm_sort(mult_f, chart, skills) {
    let perm_list = skill_perms(skills).map((ss) => [perm_mult(mult_f, chart, ss), ss]);
...
function fev_mult(chart, start = -1, end = 9999) {
...
function full_skill_mult_exact(chart, skills, options) {
    ...
    for (let [at, event] of events) {
        ...
        let base_score = (bp * base(chart) * combo_mult / chart.combo + 0.00001);
        if (options.bp)
        ...
        note_score /= 100;
        if (options.bp)
            note_score = note_score | 0;
        score += (at - last) * note_score;
        last = at;
        if (event === "combo")
            combo_mult += 0.01;
        else if (event === "skill_start")
            active.push(100 + skills[cur_skill++].mult);
        else if (event === "skill_end")
            active.shift();
...
function all_mult(chart, skills, options) {
    const mult_f = options.fever ? sl_mult_fev : sl_mult;
    return skill_perms(skills).map((order) => [full_skill_mult_exact(chart, order, options), order]);
}
